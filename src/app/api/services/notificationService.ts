...
import { NotificationForCreate } from '../types/notification/notificationForCreate';
import { NotificationForResponse } from '../types/notification/notificationForResponse';
...
export async function getListNotification(): Promise<Array<NotificationForCard> | undefined> {
    const res = await callGetRequest(`/notifications/admin`);
    if (res.status === 200) {
...
export async function deleteNotification(id: string): Promise<boolean> {
    try {
        ...
        if (result.status === 200 || result.status === 204) {
            // 204 No Content là mã trạng thái thông thường cho thành công DELETE
            return true;
        } else {
